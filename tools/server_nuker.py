...
"""
        ...
        self.ban_message = "banned by FSOCIETY"
        self.new_name = "FPERCENT was here"
        self.image_path = ""
        self.spam_message = ""
        self.channel_name_prefix = "FSOCIETY"
    ...
    def display_menu(self):
        ...
        print("\n")
        print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("nuker by FPERCENT")))
        print("\n")
    ...
    def get_inputs(self):
        ...
        self.ban_message = input("Enter the ban message (default: 'banned by FPERCENT'): ").strip()
        if not self.ban_message:
            self.ban_message = "banned by FSOCIETY"
        self.new_name = input("Enter the new server name (default: 'FPERCENT was here'): ").strip()
    ...
    def run_bot(self):
        ...
        async def on_ready():
            ...
            while True:
                try:
                    choice_input = input(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("\nEnter the number of the target server: "))).strip()
                    choice = int(choice_input) - 1
                    if 0 <= choice < len(self.bot.guilds):
                        self.target_guild_id = self.bot.guilds[choice].id
                        break
                    else:
                        print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("[!] Invalid choice. Please enter a valid number.")))
                except ValueError:
                    print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("[!] Invalid input. Please enter a number.")))
            target_guild = self.bot.get_guild(self.target_guild_id)
            if target_guild:
                confirm = input(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter(f"\n[!!!] ARE YOU SURE YOU WANT TO NUKE '{target_guild.name}'? THIS ACTION IS IRREVERSIBLE! (type 'yes' to confirm): "))).strip().lower()
                if confirm == 'yes':
                ...
                print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("[!] Could not find the selected guild. Exiting.")))
                input(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("\nPress ENTER to exit...")))
                await self.bot.close()
