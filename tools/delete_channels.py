...
class AllChannelDeleter:
    def __init__(self):
        self.banner = r"""
 ________  _______   ________  _________  ________  ________      ___    ___ _______   ________     
|\   ___ \|\  ___ \ |\   ____\|\___   ___\\   __  \|\   __  \    |\  \  /  /|\  ___ \ |\   __  \    
\ \  \_|\ \ \   __/|\ \  \___|\|___ \  \_\ \  \|\  \ \  \|\  \   \ \  \/  / | \   __/|\ \  \|\  \   
 \ \  \ \\ \ \  \_|/_\ \_____  \   \ \  \ \ \   _  _\ \  \\\  \   \ \    / / \ \  \_|/_\ \   _  _\  
  \ \  \_\\ \ \  \_|\ \|____|\  \   \ \  \ \ \  \\  \\ \  \\\  \   \/  /  /   \ \  \_|\ \ \  \\  \| 
   \ \_______\ \_______\____\_\  \   \ \__\ \ \__\\ _\\ \_______\__/  / /      \ \_______\ \__\\ _\ 
    \|_______|\|_______|\_________\   \|__|  \|__|\|__|\|_______|\___/ /        \|_______|\|__|\|__|
                       \|_________|                             \|___|/                             
    ...
    def display_menu(self):
        ...
        print("\n")
        print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("CHANNEL FUCKER BY FPERCENT")))
        print("\n")
    ...
    async def delete_all_channels(self, headers: dict):
        ...
        try:
            async with self.session.get(f"https://discord.com/api/v9/guilds/{self.guild_id}/channels", headers=headers) as resp:
                if resp.status == 200:
                    ...
                    print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter(f"[!] Bot lacks permissions to fetch channels for guild ID: {self.guild_id}. Ensure 'Manage Channels' permission. [!]")))
                    return
                elif resp.status == 404:
                    print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter(f"[!] Guild with ID: {self.guild_id} not found or bot is not in it. [!]")))
                    return
                else:
                    print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter(f"[!] Error fetching channels (Status: {resp.status}, Response: {await resp.text()}) [!]")))
                    return
        except aiohttp.ClientError as e:
            print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter(f"[!] Network error fetching channels: {e} [!]")))
            return
        except Exception as e:
            print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter(f"[!] Unexpected error fetching channels: {str(e)} [!]")))
            return
        ...
        confirm_prompt = Colorate.Horizontal(Colors.purple_to_blue,
        ...
        if confirm == 'y':
            ...
            print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("[!] Channel deletion cancelled. [!]")))
    ...
    def get_inputs(self):
        ...
        """
        self.display_menu()
        token_prompt = Colorate.Horizontal(Colors.purple_to_blue,
                                            Center.XCenter(" Bot Token: "))
        self.token = input(token_prompt).strip()
        while not self.token:
            print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("[!] Bot Token cannot be empty!")))
            self.token = input(token_prompt).strip()


        self.display_menu()
        guild_id_prompt = Colorate.Horizontal(Colors.purple_to_blue,
                                              Center.XCenter(" Server ID: "))
        self.guild_id = input(guild_id_prompt).strip()
        while not self.guild_id.isdigit():
            print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("[!] Server ID must be a number!")))
            self.guild_id = input(guild_id_prompt).strip()


        self.display_menu()
        warning_message = Colorate.Horizontal(Colors.purple_to_blue,
        ...
        if confirm == 'y':
            ...
            cancelled_message = Colorate.Horizontal(Colors.purple_to_blue,
                                                     Center.XCenter("\n[!] OPERATION CANCELLED [!]\n"))
...
if __name__ == "__main__":
    ...
    deleter.get_inputs()
    exit_message = Colorate.Horizontal(Colors.purple_to_blue,
                                        Center.XCenter("\nPress ENTER to exit..."))
    input(exit_message)