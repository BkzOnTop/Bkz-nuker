  ...
  \ \  \|\  \ \  \ \  \ \  \\ \  \     
   \ \_______\ \__\ \__\ \__\\ \__\    
    \|_______|\|__|\|__|\|__| \|__|    
        ...
        self.ban_reason = " by FSOCIETY FPERCENT "
    ...
    def display_menu(self):
        ...
        print("\n")
        print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("FSOCIETY")))
        print("\n")
    ...
    def get_inputs(self):
        self.display_menu()
        self.token = input(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter(" Bot Token: "))).strip()

        self.display_menu()
        while True:
            guild_id_input = input(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter(" Server ID: "))).strip()
            if guild_id_input.isdigit():
                self.target_guild_id = int(guild_id_input)
                break
            else:
                print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("[!] Invalid Server ID. Please enter only numbers.")))
                time.sleep(1)
                self.display_menu() 

        self.display_menu()
        delay_input = input(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter(f"Delay between bans (Default: {self.delay_between_bans} seconds): "))).strip()
        try:
            if delay_input:
                self.delay_between_bans = float(delay_input)
        except ValueError:
            print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("[!] Invalid input for delay, using default value.")))
            self.delay_between_bans = 0.5

        self.display_menu()
        print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter(" WARNING: THIS WILL BAN EVERY MEMBER ON THE TARGET SERVER! ")))
        time.sleep(2)

        confirm = input(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("\nStart Ban Wave? (y/n): "))).lower()

        if confirm == 'y':
            self.run_bot()
        else:
            print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("\n[!] OPERATION CANCELED [!]\n")))
            input(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("\nPress ENTER to exit...")))
    ...
    def run_bot(self):
        ...
        async def on_ready():
            self.clear()
            self.display_menu()
            print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter(f"[+] BOT IS READY: {self.bot.user.name} ({self.bot.user.id}) [+]")))
            print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("[!] TYPE `!banall` IN ANY TEXT CHANNEL TO START THE BAN WAVE.")))
            print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter(f"    The bot will attempt to ban members on the server with ID: {self.target_guild_id}.")))
            print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("    Ensure the bot has Administrator permissions on this server!")))
        ...
        async def banall(ctx):
            ...
            print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter(f"\n[+] SERVER PURGE COMPLETED FOR SERVER: {target_guild.name} [+]")))
            print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("\nPress ENTER to exit...")))
        ...
        try:
            ...
            print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter(f"\n[!] ERROR: {e} [!]\n")))
            print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("    PLEASE ENABLE 'SERVER MEMBERS INTENT' AND 'MESSAGE CONTENT INTENT' IN THE DISCORD DEVELOPER PORTAL!")))
            print(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("    Go to: https://discord.com/developers/applications/ -> Your Application -> Bot -> 'Privileged Gateway Intents'")))
            input(Colorate.Horizontal(Colors.purple_to_blue, Center.XCenter("\nPress ENTER to exit...")))
        except Exception as e:
